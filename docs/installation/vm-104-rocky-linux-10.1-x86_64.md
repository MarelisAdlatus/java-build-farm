...
This document describes a **repeatable installation and configuration procedure** for a Rocky Linux 10 virtual machine running on **Proxmox**, prepared as part of a larger VM farm.
...
- suitable for enterprise Linux workloads and automation
...
cores: 2
cpu: x86-64-v3
efidisk0: ssd-data:104/vm-104-disk-0.qcow2,efitype=4m,ms-cert=2023w,pre-enrolled-keys=1,size=528K
ide2: none,media=cdrom
machine: q35
memory: 2048
name: RockyLinux
net0: virtio=BC:24:11:B3:D0:1A,bridge=vmbr0,firewall=1
numa: 0
ostype: l26
scsi0: ssd-data:104/vm-104-disk-1.qcow2,iothread=1,size=32G,ssd=1
...
```bash
sudo hostnamectl set-hostname vm-rocky-linux
```
...
```bash
ssh marelis@192.168.88.104
```

Expected result:

```text
Rocky Linux release 10.1

marelis@vm-rocky-linux:~$ whoami
marelis
```
